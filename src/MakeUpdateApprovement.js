...
import ApproveUpdateRequest from "./axios/ApproveUpdateRequest";
import RejectUpdateRequest from "./axios/RejectUpdateRequest";

class MakeUpdateApprovement extends React.Component{
    state = {
        user: null,
        type: 0,
        requests: [],
    ...
    componentDidMount(){
        ...
        axios.post(`http://localhost:8080/get_requested_updates`, GLOBAL.userG )
            .then(res => {
                console.log(res);
    ...
    approve_delete(app_to_delete) {
        ApproveUpdateRequest({editor_id: this.state.editor_id, app_id: app_to_delete.app_id});
    ...
    reject_delete(app_to_delete) {
        RejectUpdateRequest({editor_id: this.state.editor_id, app_id: app_to_delete.app_id});
        this.setState(prevState => ({
                requests: prevState.requests.filter(app => app != app_to_delete)
            })
        );
                                ...
                                <ListGroup>
                                    {
                                        ...
                                        this.state.requests.map((app, index) => <ListGroup.Item action href={"#request" + (index + 1)}> {"Update Request " + (index + 1) }</ListGroup.Item>)}
...
export default withRouter(MakeUpdateApprovement);
